...
from hta.configs.default_values import AttributeSpec
from hta.configs.event_args_yaml_parser import parse_event_args_yaml, YamlVersion
...
class ParserConfig:
    ...
    ARGS_INPUT_SHAPE: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_INPUT_SHAPE
    ARGS_BANDWIDTH: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_BANDWIDTH
    ARGS_SYNC: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_SYNC
    ARGS_MINIMUM: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_MINIMUM
    ARGS_COMPLETE: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_COMPLETE
    ARGS_INFO: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_INFO
    ARGS_COMMUNICATION: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_COMMUNICATION
    ARGS_DEFAULT: List[AttributeSpec] = parse_event_args_yaml(
        DEFAULT_PARSE_VERSION
    ).ARGS_DEFAULT
    ...
    def get_minimum_args(
        ...
        return parse_event_args_yaml(YamlVersion.v1_0_0).ARGS_MINIMUM.copy()

    @classmethod
    def get_default_args(
        cls, version: YamlVersion = DEFAULT_PARSE_VERSION
    ) -> List[AttributeSpec]:
        return parse_event_args_yaml(YamlVersion.v1_0_0).ARGS_DEFAULT.copy()

    @classmethod
    def get_info_args(
        cls, version: YamlVersion = DEFAULT_PARSE_VERSION
    ) -> List[AttributeSpec]:
        return parse_event_args_yaml(YamlVersion.v1_0_0).ARGS_INFO.copy()
    ...
    def enable_communication_args(version: YamlVersion = DEFAULT_PARSE_VERSION) -> None:
        _DEFAULT_PARSER_CONFIG.add_args(
            parse_event_args_yaml(YamlVersion.v1_0_0).ARGS_COMMUNICATION
        )
    ...
    def set_global_parser_config_version(version: YamlVersion) -> None:
        ...
        ParserConfig.ARGS_INPUT_SHAPE = parse_event_args_yaml(
            DEFAULT_PARSE_VERSION
        ).ARGS_INPUT_SHAPE
        ParserConfig.ARGS_BANDWIDTH = parse_event_args_yaml(
            DEFAULT_PARSE_VERSION
        ).ARGS_BANDWIDTH
        ParserConfig.ARGS_SYNC = parse_event_args_yaml(DEFAULT_PARSE_VERSION).ARGS_SYNC
        ParserConfig.ARGS_MINIMUM = parse_event_args_yaml(
            DEFAULT_PARSE_VERSION
        ).ARGS_MINIMUM
        ParserConfig.ARGS_COMPLETE = parse_event_args_yaml(
            DEFAULT_PARSE_VERSION
        ).ARGS_COMPLETE
        ParserConfig.ARGS_INFO = parse_event_args_yaml(DEFAULT_PARSE_VERSION).ARGS_INFO
        ParserConfig.ARGS_COMMUNICATION = parse_event_args_yaml(
            DEFAULT_PARSE_VERSION
        ).ARGS_COMMUNICATION
        ParserConfig.ARGS_DEFAULT = parse_event_args_yaml(
            DEFAULT_PARSE_VERSION
        ).ARGS_DEFAULT
