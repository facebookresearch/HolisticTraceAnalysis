...
class CPGraph(nx.DiGraph):
    ...
    def _create_event_nodes(self) -> None:
        ...
        nodes_df = cast(pd.DataFrame, pd.concat([ops_df_start, ops_df_end]))
        nodes_df = (
            nodes_df.sort_values(by=["ts", "ev_idx"], axis=0)
    ...
    def _get_cuda_event_record_df(self) -> Optional[pd.DataFrame]:
        ...
        cuda_record_calls = cast(pd.DataFrame, pd.concat(cuda_record_dfs, axis=0))
        cuda_record_calls = cuda_record_calls.sort_values(by="ts", axis=0)
    ...
    def _get_cuda_stream_wait_event_df(self) -> Optional[pd.DataFrame]:
        ...
        cuda_stream_wait_events = cuda_stream_wait_events.sort_values(by="ts", axis=0)
