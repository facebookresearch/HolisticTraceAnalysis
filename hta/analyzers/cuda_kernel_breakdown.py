...
class CudaKernelBreakdown:
    ...
    def get_cuda_kernel_stats(
        ...
        extra_kernel_args: List[str] = ['grid', 'input_dims', 'warps_per_sm', 'blocks_per_sm'],
        ...
        for rank in t.traces:
            ...
            demand_index = list(set(extra_kernel_args + ['s_cat', 's_name', 'dur']))
            filter_index = list(set(demand_index) & set(rank_df.columns.values.tolist()))

            filtered_rank_df = rank_df[filter_index].copy()
            filtered_rank_df['rank'] = rank
            filtered_rank_df["kernel_type"] = filtered_rank_df[["s_name"]].apply(lambda x: classify_kernel(x["s_name"]), axis=1)

            total_cuda_df = pd.concat([total_cuda_df, filtered_rank_df], ignore_index=True)

        if not keep_unknown:
            total_cuda_df = total_cuda_df[total_cuda_df["kernel_type"] != CUDA_KERNEL_CATEGORY_NAMES.UNKNOWN]

        kernel_stats_df = total_cuda_df.groupby(by=["s_name", "kernel_type"])["dur"].agg(["sum", "max", "min", "mean", "std", "idxmax", "idxmin"])
        kernel_stats_df["max_rank"] = kernel_stats_df["idxmax"].apply(lambda idx: total_cuda_df.loc[idx, 'rank'])
        kernel_stats_df["min_rank"] = kernel_stats_df["idxmin"].apply(lambda idx: total_cuda_df.loc[idx, 'rank'])
