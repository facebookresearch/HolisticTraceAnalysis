...
class Trace:
    ...
    def _fix_mtia_memory_kernels(self, trace_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for step in profiler_steps.itertuples():
            ...
            if mask_stream.any():
                # Convert stream column to int32 to accommodate large tid values
                if trace_df["stream"].dtype != "int32":
    ...
    def _filter_irrelevant_gpu_kernels(
        ...
        def filter_gpu_kernels_with_cpu_correlation(
            ...
            filtered_cpu_kernels = cpu_kernels[
                cpu_kernels["name"].isin(profiler_step_ids)
            ]
            last_profiler_start = (
                filtered_cpu_kernels["ts"].max()
                if not filtered_cpu_kernels.empty
                else cpu_kernels["ts"].min()
            )
            last_profiler_end = (
                filtered_cpu_kernels["end"].max()
                if not filtered_cpu_kernels.empty
