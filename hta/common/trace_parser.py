...
def infer_gpu_type(
    df: pd.DataFrame, metadata: Optional[MetaData] = None, name_set: dict[str, int] = {}
) -> str:
    """
    Infer the GPU type from the trace events DataFrame or metadata.
...
def parse_trace_dataframe(
    ...
    device_type = infer_gpu_type(df, meta, local_symbol_table.get_sym_id_map())
