...
class ParserConfigTestCase(unittest.TestCase):
    ...
    @classmethod
    def _compare_config(cls, args1: List[AttributeSpec], args2: List[AttributeSpec]) -> bool:
    ...
    def test_constructor(self):
        t_cases = [
            ...
            self._TCase(ParserConfig.get_minimum_args(), ParserConfig.get_minimum_args())
        ]

        for i, tc in enumerate(t_cases):
            cfg = ParserConfig(tc.input)
            got = cfg.get_args()
            self.assertTrue(self._compare_config(tc.expect_result, got),
                            f"case #{i}: input={tc.input} expect={tc.expect_result} got={got}")

    def test_add_args(self):
        t_cases = [
            self._TCase(ParserConfig.ARGS_CUDA_MINIMUM, ParserConfig.get_minimum_args()),
            self._TCase(ParserConfig.ARGS_INPUT_SHAPE, ParserConfig.ARGS_CUDA_MINIMUM + ParserConfig.ARGS_INPUT_SHAPE)
        ]
        for i, tc in enumerate(t_cases):
