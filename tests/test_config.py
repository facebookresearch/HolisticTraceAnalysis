...
from hta.configs.config import TraceAnalyzerConfig
from hta.configs.default_values import DEFAULT_CONFIG_FILENAME


class TraceAnalyzerConfigTestCase(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_get_default_paths(self):
        paths = TraceAnalyzerConfig.get_default_paths()
    ...
    def test_constructor_no_config_file(self):
        config = TraceAnalyzerConfig(load_default_paths=False)
        self.assertDictEqual(config.get_config(), {})

    def test_constructor_one_config_file(self):
        config = TraceAnalyzerConfig(self.test_config_path, load_default_paths=False)
